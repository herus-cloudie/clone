...
const MeetingCard = ({call , type} : {call : Call | CallRecording , type : 'previous' | 'upcoming' | 'recording'}) => {
  const [previousCallsAdditionalInfo , setPreviousCallsAdditionalInfo] = useState([]);
  
  const router = useRouter();

  useEffect(() => {
    async function getPreviousCalls(){
      const getCalls = await fetch('/api/saveCallsMember')
      const {allCall} = await getCalls.json();
      setPreviousCallsAdditionalInfo(allCall)
    }
   getPreviousCalls()
   return () => {}
  } , [])

  const matchData = previousCallsAdditionalInfo?.find(previousCall => previousCall.callId == call.id)?.users.map(item => item.image)
            ...
            type == 'upcoming' ?
              <div className="flex flex-col w-full justify-between items-center sm:flex-row-reverse">
                  ...
                  <div className="flex justify-between items-center w-full sm:w-32 gap-6">
                    <Button onClick={() => navigator.clipboard.writeText(`${process.env.NEXT_PUBLIC_BASE_URL}/meeting/${call.id}`)} className="bg-dark-4 w-1/2 sm:w-36 xl:w-32">لینک دعوت<Image alt="copy icons" className="mr-1" src={'/icons/copy.svg'} width={15} height={15}/></Button>
                    <Button onClick={() => router.push(`/meeting/${call.id}`)} className="bg-blue-1 w-1/2 sm:w-20 z-10">پیوستن</Button>
                  </div>
              </div>
            : type == 'recording' ?
...
export const Avatars = ({type , matchData} : {type ?: 'previous' , matchData : Array}) => {
  console.log(matchData)
  return(
    <div className={`w-[170px] h-[35px] flex sm:-ml-10 ${type ? '-ml-8' : 'mb-8 sm:mb-0 -ml-16'}`}>
        {
          matchData ? matchData.map((img , index) => <Image key={index} className="rounded-full -mr-3 object-contain" 
          src={img} alt="avatar image" width={35} height={35}/>) : null
        }
    </div>
  )
}
